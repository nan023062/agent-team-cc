...
| 分离维度 | 能力侧 | 业务侧 |
|---------|-------|-------|
| 存储位置 | `.claude/agents/`（soul）+ `cbim/knowledge/skills/`（能力向 skill） | `.dna/`（module.md + 可选 contract + workflows/） |
| 治理者 | HR | 架构师 |
...
| 业务规则 | 写入 `CLAUDE.md` 或 `.claude/skills/` | 写入对应模块的 `module.md` / `contract.md` / `workflows/` |
...
.dna/（树状）        → 业务全景
  ├── 有哪些模块，各自的职责边界
  ├── 模块间的接口契约（contract.md）
  ├── 架构层级与依赖方向（module.md）
  └── 沉淀下来的确定性业务流程（workflows/）
...
- **完整性**：每个模块都有 module.md（协议边界场景可选 contract.md）
- **时效性**：知识在实现前更新，不是事后补写
- **层级性**：拓扑树结构，粒度与任务自然匹配
