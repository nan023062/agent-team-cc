...
| 维度 | 传统做法 | CBIM |
|------|---------|------|
| **能力向** | 一个大而全的主 agent，soul 描述所有技能 | 多个专精 agent，每次只加载目标能力 agent 的 soul |
| **业务向** | `CLAUDE.md` 写入全部业务规则，始终在上下文 | 多个模块 `.dna/`，只加载当前任务模块的知识 |

结果：**每次任务的上下文 ≈ 一个专精 agent 的 soul + 一个模块的 .dna/**，无论项目有多少 agent 和模块。

### 少上下文 → 少幻觉 → 少 token

LLM 在长上下文中的分析质量下降是已知规律（"lost in the middle"）。上下文污染不只影响准确率——它会产生复利式的 token 损耗：

```
大而全 agent + 全量 skills 常驻上下文
  → 上下文污染 → 幻觉率上升
  → 输出错误 → 需要纠正
  → 纠正本身消耗 token
  → 多轮纠正进一步污染上下文
...
多 Agent 确实产生额外的调度 token（助手理解意图 → 构造任务描述 → 派发 → 汇总）。但这是**固定开销**，而单体大 agent 的上下文污染带来的是**随项目规模增长的变动损耗**。

> 接受固定的调度开销，换取每次任务的上下文最小化——这是 CBIM 多 Agent 设计的核心取舍。
