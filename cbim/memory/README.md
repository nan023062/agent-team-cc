...
.venv/bin/python -m memory.engine.cli write-session <transcript_path>
.venv/bin/python -m memory.engine.cli load-context

# agent 命令（手动 / skill 中使用）
.venv/bin/python -m memory.engine.cli query "查询意图" [--tier short|medium] [--top-k N] [--verbose]
.venv/bin/python -m memory.engine.cli add <path> --tier short|medium
