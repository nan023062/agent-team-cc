...
python cbim/knowledge/engine/cli.py modules init <dir> --type {root,parent,leaf} --name <name> --owner <owner> [--description "..."]
```
...
3. Create the chosen top-level module(s):
   - Single-app: `python cbim/knowledge/engine/cli.py modules init . --type root --name <project-name> --owner architect`
   - Monorepo: `python cbim/knowledge/engine/cli.py modules init packages --type parent --name <workspace> --owner architect` (substitute your workspace dir)
...
```bash
python cbim/knowledge/engine/cli.py modules reindex
```
...
3. Initialize with the chosen type:
   ```bash
   python cbim/knowledge/engine/cli.py modules init <dir> --type {parent|leaf} --name <name> --owner architect
   ```
