...
| 分离维度 | 能力侧 | 业务侧 |
|---------|-------|-------|
| 存储位置 | `.claude/agents/`（soul）+ `cbim-prompt/cbi/skills/`（能力向 skill） | `.dna/` 目录 = 模块身份标识；`module.md` = 唯一硬约束；其余全部可选 |
| 治理者 | HR | 架构师 |
...
| 操作步骤 | `.claude/skills/` 全量注册，始终占用上下文 | `cbim-prompt/cbi/skills/`（能力向）+ `.dna/workflows/`（业务向），按需读取 |
...
- **`module.md` 是唯一的硬约束** — 一个文件，YAML frontmatter（元数据）+ markdown 正文（架构），替代了原来的 `module.json` + `architecture.md` 组合。这与 `.claude/agents/<name>.md` 和 `cbim-prompt/cbi/skills/<name>/SKILL.md` 形成统一的 `frontmatter + 正文` 模式。
...
| 类型 | 归属 | 存储位置 | 治理者 | 特征 |
|------|------|---------|--------|------|
| **能力向 skill** | Agent 私有能力 | `cbim-prompt/cbi/skills/<name>/SKILL.md` | HR | 描述 agent 怎么做某类操作；可移植，放到任何项目仍有意义 |
| **业务向 skill** | 模块确定性流程 | `.dna/workflows/<name>/workflow.md` | 架构师 | 描述特定模块的业务步骤；与项目强绑定，随模块演化 |

```
.claude/
└── agents/          ← soul 文件，引用 cbim-prompt/cbi/skills/ 里的能力向 skill
                        （无 .claude/skills/，无混乱堆积）

cbim-prompt/cbi/skills/   ← 能力向 skill（HR 治理，agent 跨项目可复用）
...
- agent 能力积累验证 → HR 提炼为 `cbim-prompt/cbi/skills/`（能力向 skill）→ 内化进 soul
...
| 阶段 | 路径 | 目的 |
|------|------|------|
| **短期** | `cbim-prompt/memory/store/short/` | 原始 session 记录；主要用于近期上下文恢复，自动清理 |
| **中期** | `cbim-prompt/memory/store/medium/` | 压缩提炼后的模式摘要；去噪、保留有效信号，长期留存 |
| **知识**（核心） | `cbim-prompt/cbi/skills/` + `.dna/` | 结构化落地：能力进 skills/soul，业务进 `.dna/workflows/` |
...
| 层级 | 路径 | 生命周期 |
|------|------|---------|
| 短期 | `cbim-prompt/memory/store/short/` | 提炼后标记 `distilled`，至少保留 3 天后由 cleanup 删除；未提炼的永不自动删除 |
| 中期 | `cbim-prompt/memory/store/medium/` | 长期保留，升格至知识层后手动归档 |
...
- **按需查询** — 会话中途可通过 `cbim-prompt/memory/engine/cli.py query` 检索历史
...
store/medium/         能力模式摘要（去噪，保留有效信号）
    ↓ 固化（最核心的一步）
cbim-prompt/cbi/skills/<name>/SKILL.md   新增或更新能力向 Skill
    ↓ 多次验证后内化
...
│   └── agents/                        ← 从 cbim-prompt/cc-template/agents/ 安装
│       ├── architect/
│       │   └── architect.md
│       ├── hr/
│       │   └── hr.md
...
`module.md` 是 `.dna/` 中唯一的必需文件。它将元数据（YAML frontmatter）与架构（markdown 正文）合并在一个文件中 — 与 `.claude/agents/<name>.md` 和 `cbim-prompt/cbi/skills/<name>/SKILL.md` 形成统一的 `frontmatter + 正文` 模式。
