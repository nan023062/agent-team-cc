...
The fixes verified here:
  1. `cbim_kernel.context.project_root()` raises if its fallback walk reaches
...
# --- launcher.find_project_root ------------------------------------------


def test_launcher_find_project_root_stops_at_home(tmp_path, monkeypatch, isolated_home):
    import cbim_launcher
    sub = isolated_home / "x" / "y"
    sub.mkdir(parents=True)
    assert cbim_launcher.find_project_root(sub) is None


def test_launcher_find_project_root_finds_marker(tmp_path, monkeypatch, isolated_home):
    import cbim_launcher
    ...
    deep.mkdir(parents=True)
    assert cbim_launcher.find_project_root(deep) == proj.resolve()


def test_launcher_find_project_root_no_marker_outside_home(tmp_path, monkeypatch, isolated_home):
    """Same isolation concern as test_project_root_outside_home_no_marker:
    pytest's tmp_path lives under the real user home on Windows, which
    means a polluted real `~/.cbim/` could intercept the walk. Place the
    sandbox on the repo drive root instead."""
    import tempfile
    import cbim_launcher
    repo_drive = Path(__file__).resolve().drive + os.sep
    with tempfile.TemporaryDirectory(dir=repo_drive + "tmp" if Path(repo_drive + "tmp").exists() else repo_drive) as raw:
        sub = Path(raw) / "no_marker"
        sub.mkdir()
        assert cbim_launcher.find_project_root(sub) is None
...
def test_init_writes_to_cwd_not_home(tmp_path, monkeypatch, isolated_home):
    ...
    from cbim_kernel.engine.cli import _cmd_init
