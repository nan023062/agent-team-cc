...
本仓库同时承载 CBIM 同一套模型的两种实现。**只有 V1 基于 Claude Code，V2 完全独立。**

| 版本 | 形态 | 状态 | 位置 |
|---------|------|--------|-------|
| **V1 — 提示词版（Claude Code）** | Claude Code 提示词、agent 定义、Python 钩子。作为 Claude Code 项目运行。 | **现已可用** | `install/` + `.cbim/`（下方快速开始装的就是它） |
| **V2 — 原生 agent 引擎** | 全自研 agent 运行时——不依赖 Claude Code。C# / .NET 8 独立运行时 + Avalonia UI；确定性状态机调度 + 强类型工具协议。 | **即将推出** | [`CBIM/`](CBIM/) — 设计规格与架构白皮书 |

V1 在 Claude Code 生态内验证了 CBIM 的理论（能力-业务独立 + 记忆分页）；V2 把同一套模型实现成独立运行时，让上下文剪裁、派发路由和状态变更从概率性变为确定性——同时让框架不再依赖任何宿主。
