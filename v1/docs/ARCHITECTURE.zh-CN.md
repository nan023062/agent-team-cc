...
| 操作步骤 | `.claude/skills/` 全量注册，始终占用上下文 | `.cbim/cbi/skills/`（能力向）+ `.dna/workflows/`（业务向），按需读取 |
...
- **`module.md` 是唯一的硬约束** — 一个文件，YAML frontmatter（元数据）+ markdown 正文（架构），替代了原来的 `module.json` + `architecture.md` 组合。这与 `.claude/agents/<name>.md` 和 `.cbim/cbi/skills/<name>/skill.py` 形成统一的 `frontmatter + 正文` 模式。
...
```
.claude/
└── agents/          ← soul 文件，引用 .cbim/cbi/skills/ 里的能力向 skill
                        （无 .claude/skills/，无混乱堆积）

.cbim/cbi/skills/   ← 能力向 skill（HR 治理，agent 跨项目可复用）
...
- agent 能力积累验证 → HR 提炼为 `.cbim/cbi/skills/`（能力向 skill）→ 内化进 soul
...
`.dna/` 是**模块**，散落在代码任意深度的子目录下，按文件系统层级形成模块树。项目根**不需要**一定有 `.dna/`。唯一硬要求是框架管理的注册表 `.cbim/index.md`（install 时创建，`init_module` 时更新）。
...
│   └── economy/.dna/                  ← 模块
│
├── .dna/                              ← 可选的「项目根模块」（单应用形态适合；
│   └── module.md                      ←   monorepo 通常省略）
│
└── .cbim/                              ← 框架本体
    ├── .dna/index.md                  ← 模块注册表（框架管，install 后必有）
    ├── .pin                           ← 项目锁定的 CBIM schema 版本（单行、gitignored）
    ├── README.md / README.zh-CN.md
    ├── config.json                    ← 本地框架配置（1.3.3 起不再持有版本 pin）
    │
    ├── cbi/                           ← 能力 + 业务知识库
    │   ├── README.md                  ← 四象限架构说明
    │   ├── agent-convention.md        ← agent 定义规范
    │   ├── dna-convention.md          ← .dna/ 内容规范
    │   ├── claude_md.py               ← CLAUDE.md 模板（助手身份）
    │   ├── agents/                    ← Agent 灵魂 + 私有 skill
    ...
    │   │   └── programmer/agent.py
    │   ├── engine/                    ← 知识 CRUD 原语
    │   │   ├── cli.py                 ← agents / modules 双域命令
    │   │   ├── agents.py
    │   │   ├── modules.py
    │   │   └── snapshot.py            ← 项目知识快照
    │   └── skills/                    ← 全局 skill（assistant + memory）
    │       ├── dispatch/skill.py      ← 助手请求分类与派发
    ...
    │   └── log_view.py
    │
    ├── hooks/                         ← 运行时钩子（由 .claude/settings.json 注册）
    │   ├── load_memory.py             ← SessionStart：快照 + 记忆注入
    │   └── write_memory.py            ← Stop：写入短期记忆
    │
    ├── memory/                        ← 记忆引擎
    │   ├── engine/                    ← Python 包（FileBackend / ChromaBackend）
    │   └── store/
    ...
    ├── docs/                          ← 架构文档
    │
    └── dashboard/                     ← 本地仪表盘服务（记忆 / 能力 / 知识 / 日志）
        ├── server.py
        ├── dashboard.py / dashboard.bat
        └── index.html / app.js / style.css
```

安装流程不再随仓库携带 installer 脚本。安装走仓库 `README.md` 中描述的 bootstrap（或在已全局安装内核 CLI 后用 `cbim install`）。升级走 `cbim update -y`（或 `/cbim_update` slash 命令）。项目 schema pin 存放在 `.cbim/.pin`，迁移命令 `cbim migrate --version <X>` 只动它。
