...
安装流程不再随仓库携带 installer 脚本。安装走 `INSTALL.md` 的复制式 SOP（或在已全局安装内核 CLI 后用 `cbim install`）。升级走 `cbim update -y`（或 `/cbim_update` slash 命令）。项目 schema pin 存放在 `.cbim/.pin`，迁移命令 `cbim migrate --version <X>` 只动它。
