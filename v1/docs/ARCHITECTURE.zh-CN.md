...
- **按需查询** —— 会话中通过 `.cbim/run memory query <args>` 查询历史
...
安装由 `/cbim_install` slash 命令驱动。它将内核下载到 `<project>/.cbim/kernel/`，生成 `.cbim/run`（POSIX）与 `.cbim/run.cmd`（Windows）启动 shim，安装 4 个核心 agent 到 `.claude/agents/`，安装 6 个 `/cbim_*` slash 命令，并将钩子与 MCP server 注册合并到 `.claude/settings.json`。shim 是唯一的运行时入口 —— `.cbim/run <subcommand>` 设置 `PYTHONPATH=<project>/.cbim/kernel` 后执行 `python -m engine <subcommand>`。没有项目级 pin，没有全局 venv，用户 PATH 上也没有 `cbim` CLI。重跑 `/cbim_install` 是幂等的，也是内核刷新/升级的方式。完整的安装 / 卸载 / 迁移流程（含从早期 `cbim-cc/` 布局的迁移）参见 `.cbim/kernel/project/commands/cbim_install.md`。
