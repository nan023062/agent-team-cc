...
- **Routes a tiny allow-list of subcommands to the installer** (`install`, `uninstall`, `use`, `versions`, `pin`). Everything else is forwarded to the kernel as-is. The launcher is stable; the kernel CLI is volatile.
...
- **`cbim upgrade` is routed to the kernel, not the installer (open question CLOSED).** `upgrade` is **REMOVED** from `INSTALLER_COMMANDS`; all `cbim upgrade *` invocations (`check`, `apply`, future subcommands) forward to the kernel's `project.upgrade` module as regular kernel subcommands. The installer's legacy `_cmd_upgrade` handler is **deleted**. The installer-routed allow-list shrinks to exactly `{install, uninstall, use, versions, pin}` — install-root mutations that pre-date or do not require a running kernel. Rationale: `upgrade` is a holistic operation that needs both project-state knowledge (the pin, `.cbim/config.json`) and install-state knowledge (`versions.json`); the kernel has clean access to both (project-state directly, install-state via `cbim version --json` subprocess per the installer Key Decision), whereas keeping it in the installer would force the installer to know about project pinning — a layering inversion. The launcher remains version-agnostic: it does not interpret `upgrade`; it simply resolves the project's pinned kernel and forwards.
