...
- **`hook` is one subcommand with an event arg, not one subcommand per event.** Hook handlers are dispatched by `hooks.dispatch(event_name)` after reading stdin JSON — engine just passes the event name through. This keeps the engine surface small as new hooks are added.
