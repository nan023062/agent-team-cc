...
keywords: []
dependencies: [v1/src/kernel/cbim_kernel/services, v1/src/kernel/cbim_kernel/memory, v1/src/kernel/cbim_kernel]
---
...
- **`engine/` is the only sub-module that owns kernel-managed mutations.** Since P3 Wave 1 its `cli.py` is an empty stub — the previous `cmd_modules_*` / `cmd_agents_*` handlers were inlined into top-level `cbim_kernel/engine/cli.py` as `_handle_dna_*` / `_handle_agent_*` private functions, which now drive everything through `cbi.resources.{DNAModule, Agent}`. `agents/` and `skills/` remain read-only resources, surfaced through `cbi.resources.{Agent, Skill}` (the latter exposes `Skill.list_builtin()` / `Skill.load_builtin()`, replacing the old `engine/cli._load_skills`).
