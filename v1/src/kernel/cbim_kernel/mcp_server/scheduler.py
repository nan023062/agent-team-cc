...
class Scheduler:
    def __init__(self, cbim_root: Path):
        from cbim_kernel.context import project_root as _project_root
        self.cbim_root = cbim_root
        self.project_root = _project_root()
        # Task code ships with the kernel package; discovered via importlib
        # against the cbim_kernel.mcp_server.tasks package (no filesystem path
        # needed — see _reload_tasks).
    ...
    def _reload_tasks(self) -> None:
        """Walk cbim_kernel.mcp_server.tasks.*.py and instantiate every Task subclass found."""
        self.tasks.clear()
        import pkgutil
        try:
            import cbim_kernel.mcp_server.tasks as tasks_pkg
        except ImportError:
            return
