...
from context import cbim_dir, kernel_root, project_root
...
def _launch(cwd: Path, no_browser: bool) -> int:
    """Spawn the dashboard server detached via `python -m engine`. Returns the child PID."""
    python = _python()
    args = [python, "-m", "engine", "dashboard"]
    if no_browser:
        args.append("--no-browser")

    # The hook may run outside the .cbim/run shim's env (CC invokes hooks
    # directly), so we must inject PYTHONPATH explicitly for `-m engine` to
    # resolve under the flat layout.
    env = os.environ.copy()
    env["PYTHONPATH"] = str(kernel_root()) + (
        os.pathsep + env["PYTHONPATH"] if env.get("PYTHONPATH") else ""
    )

    kwargs: dict = dict(cwd=str(cwd), env=env)
    if sys.platform == "win32":
