...
matching handler in hooks.*. Each handler receives the parsed event dict and
uses the top-level `context` module for path resolution.
...
def run_session_start(event: dict) -> int:
    ...
    """
    from .load_memory import main as load_memory
    from .auto_preview import main as auto_preview
    try:
        load_memory(event)
    except Exception as e:
