...
| 关系 | 方向 | 说明 |
|------|------|------|
| `tree` → `engine/core` + `actions` | 静态拼装 | `tree/main_loop.py` 用 `Sequence(...)` `Decorator(...)` 拼出 `ROOT` 常量；不参与运行时 |
| `actions` → `engine/core` | 实现 `Node` ABC | 每个 Action 是 `engine.core.Node` 的一个子类，签名 `tick(bb) -> Status` |
...
- **architect_execution 和 hr_execution 子循环现在是真实的 Python BT 子树**，挂载在 execution_root 内（通过嵌套子树组合），不再是 NodeSpec 描述器交给主 agent 心智执行。子树由 `build_architect_execution_subtree(llm)` / `build_hr_execution_subtree(llm)` 构建，其内部每个节点对应一次独立 LLM 调用（`LlmActionLeaf`）或一次确定性 Python 操作。
- **每个子循环节点单独 prompt / 单独 parse / 单独 output_field**——禁止"一次 LLM 调用包揽子循环全程"的描述器模式。intent_analyze / decompose / arch_gate 各自一个 `LlmActionLeaf`，按 `engine/core` 的一 tick 一 LLM 调用铁律落实。
- **DispatchArchitect / DispatchHR 节点不再产生 yield**。两者原先经主 agent 中转派 Architect / HR agent 文件，现已被 `ArchitectExecSubtree` / `HRExecSubtree` 子树根替代——子循环全程在引擎进程内完成。execution_root 的 yield 只剩 `DispatchWork`（Work Agent 仍需经 Claude Code Task tool 派出，这一条不变）。
...
- **NodeSpec 描述器废弃为兼容垫片**。`loops/architect_execution.py` / `loops/hr_execution.py` 中的 `NODE_SPECS` 和 `compose_prompt` 保留供单元测试与旧版工具读取，**不再进入运行时调用路径**；真正的子循环执行由上述两个 `build_*_subtree(llm)` 工厂构建并装入根树。两份描述器在下一版本删除。
