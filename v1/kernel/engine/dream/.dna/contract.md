...
| 节点 ABC、组合节点、装饰器实现 | 复用 `engine/bt/core/` 子模块内部；外部不可见、不可继承（C1 单接口原则） |
| `SequenceTolerant` 组合节点的内部实现 | 归 `engine/bt/core/composite.py`（在 bt 模块内新增）；外部只通过 `tree/` 静态拼装可见 |
...
| `trace.jsonl` 单条事件结构 | 复用 `bt/persistence/trace.py` 格式；事件类型是观测约定，不是 RPC 契约——下游消费者按 best-effort 解析 |
