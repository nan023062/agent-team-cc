...
flowchart TB
    subgraph DREAM["kernel/engine/dream（本模块）"]
        ...
        Actions["actions/<br/>Init / Mem*Step / DispatchArchGovern / DispatchHRGovern / CollectArchAdvice / CollectHRAdvice / EmitReport / Finalize"]
...
| `api` → `tree` + `engine/core` + `kernel/engine/persistence` | 入口启动 / 恢复 | `dream_tick` 读 `build_dream_root()` 启 Runner；`dream_tick_resume` 调 `persistence.snapshot.read_bb / read_resume` 后由 Runner 按 `runner_resume_path` 重建栈 |
...
| `actions` → MainAgent（yield） | 协程式 yield/resume | Architect / HR 治理派工一律走 `DreamResult.Yield(DispatchRequest)` → 主 agent Task tool → `dream_tick_resume` 路径；引擎进程内**不**持有 Architect / HR 客户端 |
