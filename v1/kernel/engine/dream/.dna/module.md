...
flowchart TB
    ...
    subgraph EXEC["kernel/engine/execution（被复用）"]
        Core["core/<br/>Node ABC · Composite · Decorator · Runner · Blackboard · 持久化"]
    end
...
| `api` → `tree` + `execution/core` | 入口启动 / 恢复 | `dream_tick` 读 `DREAM_ROOT` 启 Runner；`dream_tick_resume` 读盘后由 Runner 按 `runner_resume_path` 重建栈 |
| `persistence` | **复用 execution/core 的 Runner 持久化机制** | 不自建持久化子模块。Runner 把 `bb.json` / `trace.jsonl` / `resume.json` 写到 `.cbim/scheduler/dream/<run_id>/`（路径前缀由 api 注入），文件格式与执行循环一致，方便 dashboard / 调试工具复用 |
...
**无循环依赖**——单向自顶向下：`api → tree → {core[复用 execution], actions} → memory[内部维护接口]`。`dream` 依赖 `execution`，`execution` 不依赖 `dream`。
...
- **v1/kernel/engine/execution/core（复用）** —— Node ABC / Composite / Decorator / Runner / Blackboard / 持久化机制 / trace 文件格式全部复用。`dream/tree/` 构造的根树通过 `execution.core.Runner` 驱动；`dream/actions/` 继承 `execution.core.Node`。是本模块的核心 outbound。
...
依赖方向：`dream → execution/core`、`dream → memory.{compaction,_facade}`（内部维护接口）、`mcp_server → dream`。无环。
