...
name: dream-engine
owner: architect
description: CBIM 治理循环驱动引擎；复用 bt/core 行为树原语，承载独立的治理根树、独立黑板、独立 trace、独立入口（dream_tick / dream_tick_resume），由 SessionStart 补跑触发
keywords: []
dependencies:
  - v1/kernel/engine/bt
  - v1/kernel/memory
...
CBIM **第二个根循环**的驱动引擎，与执行循环（`engine/bt`）平级共存——不是其子循环、不是其装饰器、不是其插件。复用 `engine/bt/core` 的 Node ABC / Composite / Decorator / Runner / 持久化 / trace 原语，但持有独立的根树拓扑（DreamRoot）、独立的黑板 schema（8 字段）、独立的 trace、独立的 MCP 入口工具。
...
| 误解 | 澄清 |
|------|------|
| `engine/bt` 的子模块 | 不是。是平级第二根，与 bt 共享 `bt/core` 但持有独立根树；依赖方向 `dream → bt/core`，不反向。 |
...
flowchart TB
    ...
    subgraph BT["kernel/engine/bt（被复用）"]
...
| 关系 | 方向 | 说明 |
|------|------|------|
| `tree` → `bt/core` + `actions` | 静态拼装 | `tree/dream_root.py` 用 `Sequence(...)` `SequenceTolerant(...)` `@Trace @Timeout @Catch` 拼出 `DREAM_ROOT` 常量；不参与运行时 |
| `actions` → `bt/core` | 实现 `Node` ABC | 每个 Action 是 `bt.core.Node` 的子类，签名 `tick(bb) -> Status` |
| `api` → `tree` + `bt/core` | 入口启动 / 恢复 | `dream_tick` 读 `DREAM_ROOT` 启 Runner；`dream_tick_resume` 读盘后由 Runner 按 `runner_resume_path` 重建栈 |
| `persistence` | **复用 bt/core 的 Runner 持久化机制** | 不自建持久化子模块。Runner 把 `bb.json` / `trace.jsonl` / `resume.json` 写到 `.cbim/scheduler/dream/<run_id>/`（路径前缀由 api 注入），文件格式与 bt 一致，方便 dashboard / 调试工具复用 |
...
**无循环依赖**——单向自顶向下：`api → tree → {core[复用 bt], actions} → memory[内部维护接口]`。`dream` 依赖 `bt`，`bt` 不依赖 `dream`。
...
**为什么和 bt 平级而非套在 bt 之下**：执行循环（用户驱动）和治理循环（scheduler 驱动）承载根本不同的驱动模型。强行合并会让黑板 schema 膨胀且语义混乱；独立两根 = 两份 trace、两份审计边界、清清楚楚。共享 `bt/core` 因为行为树原语本身没有"用户驱动 / scheduler 驱动"之分。

## Key Decisions

- **治理循环是 CBIM 第二个根循环，与执行循环平级共存。** 不是子循环、不是装饰器、不是插件。两根共享同一个行为树引擎本体（`bt/core`），但各持独立根树、独立黑板、独立 trace、独立入口工具。`engine/bt` 的代码里不允许 import `engine/dream` 任何模块——单向依赖铁律。
...
- **v1/kernel/engine/bt/core（复用）** —— Node ABC / Composite / Decorator / Runner / Blackboard / 持久化机制 / trace 文件格式全部复用。`dream/tree/` 构造的根树通过 `bt.core.Runner` 驱动；`dream/actions/` 继承 `bt.core.Node`。是本模块的核心 outbound。
...
依赖方向：`dream → bt/core`、`dream → memory.{compaction,_facade}`（内部维护接口）、`mcp_server → dream`。无环。
