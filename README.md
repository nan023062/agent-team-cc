...
核心理念：**用更多模块的 `.dna/` 替代单体的 `CLAUDE.md` 和堆积的 `skills/`。**

- **能力**（Agent 定义、Skills）—— 可移植的专业技能，存于 `.claude/agents/`，由 HR 治理
- **业务**（模块知识、架构蓝图）—— 特定项目的知识图谱，存于各模块 `.dna/`，由架构师治理
- **记忆**（Memory）—— 跨会话的上下文积累，短期 + 中期，自动注入每次会话

业务向 skill 封装为模块 workflow，随模块按需加载；能力向 skill 由 agent 主动 Read，不常驻上下文。每次任务的上下文 ≈ 一个专精 agent 的 soul + 一个模块的 `.dna/`，与项目规模无关。

**上下文最小化的复利效应**：少上下文 → 少幻觉 → 少错误 → 少纠正轮次 → 更少 token。多 agent 的调度开销是固定成本，单体大 agent 的上下文污染是随项目增长的变动损耗——CBIM 用前者换后者。
