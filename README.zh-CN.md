...
2. 在 Claude 输入框运行：
   ...
   /cbim_install
   ```
   （首次安装：若 slash 命令尚未注册，请把本节末尾的「首次引导段落」粘进 Claude。它执行的步骤与 `/cbim_install` 完全相同 —— 下载 + `python3 -m engine init`。）
...
**刷新 / 升级**：重跑 `/cbim_install` —— 幂等操作。shim 与内核会被重新生成；`.dna/` 与 `.cbim/memory/` 会被保留。没有 `cbim update` CLI；slash 命令是唯一的刷新路径。
...
<details>
<summary><strong>首次引导段落</strong>（若 <code>/cbim_install</code> 尚不可用，请粘进 Claude）</summary>
