...
| 子模块 | 一句话职责 | 状态 |
|--------|----------|------|
| `Agent/` | **Agent 装配服务门面 + Brain 多脑区子结构**（旧名 AgentSystem）。包含 AgentDescription schema + 装配胶水 OpenInstance + Session 写侧 + `Brain/` 多脑区编织（含 ExternalMotorCortex · ClaudeCodeMotorCortex）。**目录名是否同步重命名为 `Agent/` 见「AgentSystem → Agent 重命名决策」节** | spec |
...
**废弃说明：** 原「Agent 层第四子模块 `ExternalAdapter/`」本轮取消。外部 AI 引擎接入能力本质上是「肉」者者「上肢伸展」，装到 Brain/MotorCortex 子类中是唯一符合语义的位置；作为与 AgentSystem 平级的「装配家」会带来「外部引擎当主脑」这种语义上合理但本质上错位的 OO 层级。
...
**已废弃**（代码已物理删 / 待物理删）：`SystemTools/`、`Kernel/ExecutionUnit/`、`Kernel/TaskRunner/`、AgentSystem 下原独立的 `McpAdapter/`、**`ExternalAdapter/`**（本轮：内容物理收编进 `Agent/Brain/MotorCortex` 家族，原顶层模块废弃 · 目录待人工删除）。
...
flowchart TD
    ...
    subgraph L2["Agent 层（虚拟人代理 · 人的能力）"]
        AS["Agent<br/>(AgentDescription 持 Memory/Tool/Skill/MCP)<br/>(OpenInstance 装配 + Session 写)<br/>+ Brain 多脑区：PrefrontalCortex / ParietalLobe /<br/>Hippocampus / MotorCortex（含 ExternalMotorCortex · ClaudeCodeMotorCortex）"]
...
- **外部 AI 引擎不是 Agent 层的独立子模块**（原 ExternalAdapter 本轮废弃）——它们以 `Agent/Brain/MotorCortex` 家族下的 `ExternalMotorCortex` 子类形式嵌入单个 Agent 的脑区。
...
老的 `_ARCHITECTURE.md` 顶层文档将由下一个 Work Agent 任务（doc_writer）重写以匹配三层模型；同步删除原 ExternalAdapter 独立顶层描述。
