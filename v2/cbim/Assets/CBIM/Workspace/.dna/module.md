...
**Workspace 是 CBIM 的 Workspace 层**（v2 三层模型中的三层之一）——管理模块树 + 模块对象。该层与 Agent 层**互不依赖**——二者是 v2 三层中两个独立资产，在 task 期由 **Agent/Brain 内部装配机制**（本轮 Kernel 顶层模块物理删除后，原 Kernel 以 ContextProvider 组合的路径下沉到 Brain 内部）以 ContextProvider 组合。
...
Agent 层：Agent（含 Brain 多脑区·含 ExternalMotorCortex 桥接外部 AI 引擎）/ Channel
```

**本模块依赖**：`CBIM.Skills` + `CBIM.Mcp` + `CBIM.Storage`（三个基建层抽象）。
**本模块不依赖**：Agent 层任何模块（包括 Agent / Channel）。**原 「ExternalAdapter」 顶层模块上轮废弃**，外部 AI 引擎接入以 `Agent/Brain/MotorCortex` 子类形式存在于单个 Agent 内部，与本模块无跨层依赖。**本轮补丁：原 Agent 层内的 `Kernel/` 子模块物理删除**（FlowGraph / TaskScheduler / ContextProviders 随代码 / .meta / .dna 一同 git rm），本模块同样不依赖。
...
二者**结构对称**：都以 `Description`（类型描述）+ `Instance`（实例运行态）二元结构组织；都直接依赖 Storage；都不互相依赖；跨维度协同由 **Agent/Brain.PrefrontalCortex 主脑**（铁律 A：唯一通路）在 Task 期组合（本轮 Kernel 顶层模块物理删除后，原 Kernel.FlowGraph 职责下沉为主脑内部 LLM + AIFunction 闭环）。

**唯一跨维度共享抽象（本轮重点）**：`McpDescriptor`——业务维度 `ModuleDescription.McpList` 与能力维度 `AgentDescription.McpList` 同类型。Workspace 上向引用 `CBIM.Mcp.McpDescriptor`，依赖方向 `Workspace → CBIM.Mcp`（基建层），符合 C3 单向依赖。

## ModuleDescription 三段式语义（本轮重要）

**v2 三层模型下，`ModuleDescription` 明确为三段式结构**——三段分别担负：

- **Metadata 段**——所有3 类文档性元数据。`ModuleMetadata Kind = Local | Remote` 两种实现（详见后面「ModuleMetadata 是纯知识载体」裁决节）。
- **Workflows 段**——`List<SkillDescriptor>` 样状语意声明业务 Skill（业务流程）；SkillDescriptor 本身是基建层抽象（`CBIM.Skills`）。
- **McpList 段**——`List<McpDescriptor>` 业务操作接入点（业务自带的 MCP server）；与能力主 AgentDescription.McpList 同抽象同类型，但实例独立，仅在 task 期装配点合并去重。
...
1. **`Metadata` 是纯知识载体**——只描述「是什么 / 规则 / SLA」，不夾带任何操作协议。
2. **`Workflows` 是业务流程语义声明**——执行实例由 **Agent/Brain 内部 PrefrontalCortex 主脑**（铁律 A：唯一通路）装配与驱动。**本轮 Kernel 顶层模块物理删除**，原「执行实例由 Kernel/FlowGraph 装配」表达随 Kernel 一同废除；未来业务 Workflow 装配下沉到主脑内部 LLM + AIFunction 闭环。
...
- `ModuleDescription.McpList` → **Agent/Brain 内部装配机制**（本轮 Kernel 顶层模块物理删除后，原「业务 Workflow / Kernel.ContextProviders 装配」路径下沉为 Brain 内部装配机制职责）按需启动 + 包 AIFunction → 挂当前 task 上下文。
...
flowchart TD
    ...
    WF["Workflow\n(业务流程语义)"]
    MCP["CBIM.Mcp.McpDescriptor\n(跨维度共享抽象·基建层)"]
    Storage["CBIM.Storage"]
    Brain["Agent/Brain 内部装配机制\n(原 Kernel/ContextProviders.WorkspaceContextProvider 职责下沉)"]
    ...
    MD -- 跨维度引用 --> MCP
    WS --> Storage
    Brain -. 读 ModuleDescription / 实例 .-> WS
    ...
    class MCP cross;
    class Brain external;
```

依赖方向：Workspace → `CBIM.Mcp`（单向引基建层 Mcp 抽象；Mcp 不反向引 Workspace）。本轮删除 `Owners` 后，业务侧 → 能力侧的「字符串反向引用 AgentDescription.Id」也随之消失。**本轮补丁**：原 mermaid 中 `WCP["Kernel.ContextProviders.WorkspaceContextProvider"]` 节点随 Kernel 顶层模块物理删除一同去除；该职责下沉为 Agent/Brain 内部装配机制（以上图中 `Brain` 节点表示），读侧接口不变。
...
- **不依赖** Agent 层任何模块（Agent / Channel）。Agent 层 ⊥ Workspace 层，仅由组合根 / **Agent/Brain 内部装配机制**（本轮 Kernel 顶层模块物理删除后，原 Kernel 在 task 期组合的职责下沉到 Brain 内部）在 task 期组合。**原 「ExternalAdapter」 顶层模块上轮废弃**（外部 AI 引擎以 `Agent/Brain/ExternalMotorCortex` 子类形式存在于单个 Agent 内部），本模块不依赖。**原 `Kernel/` 子模块本轮随代码 / .meta / .dna 一同 git rm 物理删除**，本模块从未依赖。
...
- **MCP 接入点走 C# 层 `ModuleDescription.McpList`**——不再在 `ModuleMetadata` 文档侧夾带。
- **共用 `CBIM.Mcp.McpDescriptor` 但不反向依赖能力侧**——跨维度共享是抽象复用，不是耦合。本轮 Mcp 顶层化后，依赖表达从「依赖 AgentSystem.Mcp」调正为「依赖基建层 CBIM.Mcp」。
...
- **模块上下文装配由 Agent/Brain 内部装配机制负责**（本轮新增）——原 `Kernel/ContextProviders.WorkspaceContextProvider` 随 Kernel 顶层模块物理删除。读侧接口由 WorkspaceService 公开；调用方下沉为 Agent/Brain 内部装配机制，本模块不反向感知 Brain。
...
- 不持有 Agent / 记忆数据。
- **不持有工具声明**——本轮裁决。
- **不含 MCP 启 / 停 / 接连胶水**——装配胶水在能力侧（AgentSystem.OpenInstance）或 **Agent/Brain 内部装配机制**（本轮 Kernel 顶层模块物理删除后，原「业务 Workflow（Kernel.FlowGraph / ContextProviders）」路径下沉为 Brain 内部装配机制职责）中调 `Microsoft.Agents.AI.Mcp` client。
