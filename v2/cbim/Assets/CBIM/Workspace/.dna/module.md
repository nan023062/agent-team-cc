...
| ModuleDescription（业务）| Metadata（文档）| Workflows（流程声明）| McpList（操作接入点）| **Owners（负责人编制）** |
...
flowchart TD
    ...
    MD["ModuleDescription\n(Id + Name + Metadata + Workflows + McpList + Owners)"]
...
| 概念 | 形态 | 存储 |
|------|------|------|
| **ModuleDescription** | 模块「类型」：Id / Name / Metadata / Workflows / McpList / **Owners** | 代码实例化；Metadata 为 Local 时点向 `<project>/<path>/.dna/module.md` |
