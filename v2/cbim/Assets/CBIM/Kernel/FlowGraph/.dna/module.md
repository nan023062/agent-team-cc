...
/// <summary>把一条 CbimTask 转一次 AIAgent.RunAsync 的 Executor。</summary>
public sealed class CbimTaskExecutor : Executor<CbimTask, AgentRunResponse>
{
    public CbimTaskExecutor(IAgentSystemSessionWriter sessionWriter);
    protected override ValueTask<AgentRunResponse> HandleAsync(CbimTask task, IWorkflowContext ctx);
}

/// <summary>业务 Workflow 装配类示范（具体业务 Workflow 各自一文件）。</summary>
public static class ChatWorkflow
{
    public static Workflow Build(AIAgent classifyAgent, AIAgent respondAgent, IAgentSystemSessionWriter sessionWriter);
}
```

**关键点**：`CbimTaskExecutor` 内部调 `task.Who.RunAsync(...)`、调 `sessionWriter.AppendSessionEvent(...)`——这就是 CBIM 与 Microsoft 的唯一接点。原 TaskRunner 的「拼参 + 调 RunAsync + 写 Session」三步全部在这个 Executor 的 `HandleAsync` 中完成，不再需要独立的 TaskRunner 模块。

### Task 期 MCP / 工具透传

CbimTaskExecutor **不介入工具 / MCP 装配本身**。本轮重大修正：工具装配的数据源从 `task.Where`（module 列表）改为 `task.Who`（agent、能力维度）。

**装配发生在哪里**：AgentSystem.OpenInstance 装配 AIAgent 时，读 `AgentDescription.Tools` + `AgentDescription.AgentExtensionClis` → 按 per-agent 沙盒注入 AIFunction。装配后的 AIAgent（已带工具）被作为 `task.Who` 传入 CbimTaskExecutor。

**Executor 的透传职责**：

- CbimTaskExecutor 拿到 `CbimTask` 后直接调 `task.Who.RunAsync(...)`——`task.Who` 本身已是装配完成、工具已挂的 `AIAgent`。Executor **不负责重新装配**。
- 若某些场景 Executor 需要「在 Task 期内造补充 agent 实例」，可调 `AgentSystem.OpenInstance(name, options)`；传入的信息是「agent 名 / chat client 覆盖项」，**不传 module 列表**——module 列表只是业务语义，不是装配语义。
...
| 项 | 上一轮（错） | 本轮（正） |
|---|---|---|
| Executor 透传什么 | `task.Where` → `OpenInstanceOptions.ActiveModulePaths` | 什么也不透传——`task.Who` 本身自带工具 |
| 装配数据源 | ModuleDescription.standard_tools | AgentDescription.tools |
| 装配发起点 | Task 期动态（按 module 拼装） | OpenInstance 期静态（按 agent 装配一次） |
| 动态性体现 | 同一 agent 跨 module 工具不同 | 同一 task 选不同 agent 则工具不同，间接动态 |

**为什么这是正确的**：

- 「谁能调工具」是 agent 的能力描述，定义在 AgentDescription；Executor 拿到 `task.Who` 时能力已函盖在 AIAgent 中。
- 「装配唯一胶水点 = OpenInstance」铁律不动，但其输入是 agent 自身声明而非 task.Where。
- Executor 职责收窄为「跳一下 RunAsync + 写 Session」三件事，更加纯净。

## Dependencies

- **`Microsoft.Agents.AI.Workflows`**——核心运行时（Workflow / Executor / Edge / WorkflowHost）。
- **`Microsoft.Agents.AI`**——`AIAgent` / `AgentRunResponse`。
- **`CBIM.Kernel.TaskScheduler`**——`CbimTask` 数据类。
- **`CBIM.Kernel.ContextProviders`**——装配 AIContextProvider 注入 RunAsync。
- **`CBIM.AgentSystem`**——通过 `IAgentSystemSessionWriter` 接口写 Session（避免反向耦合，接口归属 AgentSystem）。
