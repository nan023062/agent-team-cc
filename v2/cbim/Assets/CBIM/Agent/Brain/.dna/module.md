...
```
BrainBase                       (abstract · 已含 msai 装配 · 所有脑区天生 LLM 思维链)
   ├── PrefrontalCortex         (主脑 · 直接继承基类 · Channel.SendAsync 投递目标)
   ├── ParietalLobe             (架构脑 · 直接继承基类)
   ├── Hippocampus              (记忆学习脑 · 直接继承基类)
   └── MotorCortex              (运动皮层 · 抽象 · 唯一 Native/External 分支点)
          ├── NativeMotorCortex      (用 msai 装配 · 默认 1 个)
          └── ExternalMotorCortex    (桥接外部 agent 引擎 · 抽象)
...
| 上一轮（已废） | 本轮 |
|---------------|------|
| BrainBase + NativeBrain + ExternalBrain 三层抽象 | **BrainBase（已含 msai）+ MotorCortex 一层（Native / External 仅在皮层下分）** |
...
| NativeBrain 中间层重复装配 msai | **基类已含 msai 装配，无需中间层** |
...
上一轮 BrainBase 不持 msai，仅 NativeBrain 持。本轮认识到：**所有 CBIM 脑区都需要 LLM 思维链能力**——主脑要推理「该调谁」、架构脑要推理「这模块设计合理吗」、Hippocampus 要推理「这条记忆该提炼成什么模式」、NativeMotorCortex 要推理「调哪个工具 + 怎么调」。**全部都是 LLM 任务**——没有「不需要 LLM 的脑区」。

既然 100% 的具体脑区都需要 msai，把 msai 装配下沉到基类是最自然的设计：

- 取消了 NativeBrain 中间层（它的唯一职责就是「装配 msai」——这职责现在归基类）。
- ExternalMotorCortex 是基类规则的例外：它不通过 msai 装配（外部引擎自带 LLM），但仍继承基类——它持有的 `Agent` 字段为 null 或指向「桥接 stub」是子类自决。
...
/// 脑区契约公共基类。
///
/// 本轮重要变动：基类直接持 msai 装配的 ChatClientAgent + IChatClient——
/// 所有具体脑区天生具备 LLM 思维链 + Function Invoking 闭环。
/// 唯一例外：ExternalMotorCortex 走桥接路径（Agent 字段语义重定义）。
public abstract class BrainBase : IAsyncDisposable
{
    ...
    /// msai 运行体——基类持有，所有脑区天生具备 LLM 思维链能力。
    /// ChatClientAgent 默认实现，由 BrainBase 构造器装配：
    ///   - 拿 BrainDescriptor.Soul → Instructions
    ///   - 拿 BrainDescriptor.IChatClient → 底层 LLM
    ///   - 包 FunctionInvokingChatClient → 启用 AIFunction 调用闭环
    ///   - 挂 BrainDescriptor 中的 SystemTools / Skills / McpList 派生的 AIFunction
    /// ExternalMotorCortex 子类在构造期可重定义为 null 或桥接 stub。
    public AIAgent Agent { get; }

    /// 主脑回调协议——子脑区通过该回调向 PrefrontalCortex 汇报结果。
    /// 极小化设计：只 ReportProgress / ReportOutcome，不允许反向调度。
    protected IPrefrontalCallback PrefrontalCallback { get; }

    /// 投递子任务到本脑区。
    /// 默认实现：包成 ChatMessage 投给 Agent.RunAsync，转译产出为 BrainOutcome。
    /// ExternalMotorCortex 子类重写为「Adapter.SubmitAsync + AwaitResultAsync」路径。
    public virtual Task<BrainOutcome> InvokeAsync(BrainInvocation invocation, CancellationToken ct);

    public abstract ValueTask DisposeAsync();
}

/// PrefrontalCortex 暴露给其他脑区的回调面（极小化）。
/// 脑区只能「回报」，不能「下发」——主脑唯一调度铁律的物理护栏。
...
- `BrainBase.Agent` 由基类构造器装配，子类不重新装配——这是「msai 装配收敛到基类」的物理落地。
- `BrainBase.InvokeAsync` 提供默认实现（包 ChatMessage → Agent.RunAsync），子类一般不重写；ExternalMotorCortex 是唯一需要重写的子类（走桥接路径）。
- `IPrefrontalCallback` 故意极小——所有跨脑区数据流必须经 PrefrontalCortex 中转，这是主脑唯一调度铁律。
...
{
    /// 可调度的其他脑区清单（装配期注入）。每个会被包装为 __brain_call_* AIFunction。
...
**__brain_call_\* 注入机制**（装配期）：

```
foreach callable in instance.Brains where callable is not PrefrontalCortex:
    fn = AIFunctionFactory.Create(
        name: $"__brain_call_{callable.BrainId.Replace('.', '_').Replace('-', '_')}",
        description: $"Dispatch sub-task to brain '{callable.BrainId}'. "
                     + $"Use when: {callable.RoleDescription}.",
        parameters: [intent, context?, structured?],
        handler: async (args, ct) => {
            var outcome = await callable.InvokeAsync(
                new BrainInvocation(Guid.NewGuid().ToString(), args.intent, args.structured, ...),
                ct);
            return outcome.Summary;   // 作为 ToolMessage 回填 LLM
        }
    );
    prefrontal.Agent.AIFunctions.Add(fn);
```

**默认 CallableBrains 清单**：`__brain_call_parietal_lobe` / `__brain_call_hippocampus` / `__brain_call_motor_cortex_native` (装了 ClaudeCode 后再多一个 `__brain_call_motor_cortex_claude_code`)。
