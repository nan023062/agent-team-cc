...
class Scheduler:
    def __init__(self, cbim_root: Path):
        ...
        self.project_root = cbim_root.parent
        self.tasks_dir = cbim_root / "mcp_server" / "tasks"
    ...
    def _reload_tasks(self) -> None:
        """Walk .cbim/mcp_server/tasks/*.py and instantiate every Task subclass found."""
        self.tasks.clear()
        if not self.tasks_dir.exists():
            return
        for path in sorted(self.tasks_dir.glob("*.py")):
            if path.name.startswith("_"):
                continue
            mod_name = f"mcp_server.tasks.{path.stem}"
            try:
                mod = importlib.import_module(mod_name)
            except Exception as exc:
                self._log("SCHED", f"failed to import {path.name}: {exc}")
                continue
    ...
    def _log(self, tag: str, msg: str) -> None:
        try:
            sys.path.insert(0, str(self.cbim_root))
