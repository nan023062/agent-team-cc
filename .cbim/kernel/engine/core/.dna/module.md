...
- **决策类原语自追踪 `*_decision` 事件。** `SwitchBranch` 在 `key_fn(bb)` 求值后写一条 `switch_decision`（含 `key` / `matched_case` ∈ {命中 case 名 / `__default__` / `__no_match__` / `__error__`} / `chosen_child`）；`Selector` 在每个出口（首子 SUCCESS / 中间 RUNNING / 全 FAILURE）写一条 `selector_decision`（含 `child_results` 数组 + `chosen_child` + `outcome` ∈ {`success` / `running` / `all_failure`}）。两者都追加到同一份 `bb.trace`，由 Runner 出口点统一刷盘——与节点三事件、LLM 三事件共享缓冲区与刷盘契约，节点对象仍无跨 tick 状态。提取 `core/_trace_utils.py` 私有辅助模块承载 `_append_trace_event` / `_now_iso_ms`，让 `composite.py` 与 `runner.py` 共用同一份实现，避免反向 import。
