...
on first tick when ``bb.hr_governance_dispatched`` is None. On resume the
sibling CollectHRAdvice node owns ``on_resume``.

Prompt scaffolding is delegated to ``engine.dream.loops.hr_governance`` so
the design flowchart (WORKFLOW-HR.zh-CN.md §2 六类扫描) NodeSpec list is
the single source of truth.

Reference pattern: ``engine.execution.actions.dispatch_core_agent.DispatchCoreAgent``.
Mirror of ``dispatch_arch.DispatchArchGovern`` — agent_type / subtask_id /
flag field are the only differences.
"""
