...
def read_log(cbim: Path | None = None, since: int = 0) -> dict:
    ...
    """
    # Lazy import: session_log lives in engine/ and the dashboard process
    # already has .cbim/ on sys.path, but we keep the import inside the
    # function so import errors during package discovery don't break the
