...
│   │   ├── architect.md
│   │   └── programmer.md
│   ├── skills/                     # 用户自定义 skill（内置 skill 编译进扩展，不在此处）
│   │   └── <name>/
...
## 七、能力-业务对称设计（Capability-Business Symmetry）

CBIM 的两层组织——能力层（Capability）和业务层（Business）——具有结构对称性。这不是巧合，而是框架哲学的核心。

### 7.1 对称表

| 能力层 (Capability) | 业务层 (Business) | 关系 |
|-----|-----|------|
| **Agent** | **Module** | Agent 是执行主体；Module 是知识载体 |
| **Skill** (跟着 agent 走) | **Workflow** (跟着模块走) | Skill = "我会做 X"（可迁移能力）；Workflow = "在这里这样做 X"（场景化流程） |
| `.cbim/skills/<name>/SKILL.md` | `.dna/workflows/<name>/workflow.md` | 存储位置对称 |
| `cbim_skill_*` tools (5 个) | `cbim_workflow_*` tools (5 个) | Tool 接口对称 |

### 7.2 为什么这层对称重要

1. **概念正交**：Agent 是"谁来做"，Module 是"做什么"。Skill 是 agent 的能力描述，Workflow 是 module 的流程描述。两者不混淆。
2. **统一心智模型**：学会一侧的加载模式，另一侧自然理解。降低学习成本。
3. **所有权清晰**：Skill 跟 agent 走（agent 归档，skill 跟着归档）。Workflow 跟模块走（模块迁移，workflow 跟着迁移）。没有孤儿文件。
4. **Tool 设计一致**：`cbim_skill_list/get/create/update/delete` 与 `cbim_workflow_list/get/create/update/delete` 完全对称——同样的动词，同样的模式。

### 7.3 加载生命周期对照

两侧遵循相同的 **eager frontmatter / lazy content** 加载模式：

```
                    Eager (进入作用域)          Lazy (命中时加载)
                    ─────────────────          ────────────────
Skill:              cbim_skill_list()          cbim_skill_get(name)
                    → frontmatter[]            → frontmatter + content

Workflow:           cbim_module_get(path)      cbim_workflow_get(path, name)
                    → module.workflows[]       → frontmatter + content
                      (仅 frontmatter)
```

**Frontmatter 格式一致**（SKILL.md 和 workflow.md 共用同一组字段）：
...
### 7.4 与 `cbim_*` tools 的对应关系

| Capability Tool | Business Tool | 说明 |
|----------------|--------------|------|
| `cbim_skill_list()` | `cbim_workflow_list(modulePath)` | 列出 frontmatter 数组 |
| `cbim_skill_get(name)` | `cbim_workflow_get(modulePath, name)` | 加载完整 content |
| `cbim_skill_create(...)` | `cbim_workflow_create(...)` | 创建新条目 |
| `cbim_skill_update(...)` | `cbim_workflow_update(...)` | 部分更新 |
| `cbim_skill_delete(name)` | `cbim_workflow_delete(modulePath, name)` | 删除条目 |

注意 workflow 的每个操作都带 `modulePath` 参数——因为 workflow 跟着模块走，必须知道"在哪个模块的 workflow"。Skill 不需要，因为 skill 的存储位置是项目级的 `.cbim/skills/`。

### 7.5 与封闭性原则的关系

封闭性（§八）是**横向约束**——控制"谁能访问什么"。对称是**纵向结构**——定义"事物如何组织"。两者正交互补：

- 封闭性决定 architect 可以 CRUD workflow，programmer 只能 list/get workflow
- 对称决定 workflow 的 CRUD 接口与 skill 的 CRUD 接口形状一致
...
| **architect** | `['Read', 'Glob', 'Grep', 'mcp__cbim__module_*', 'mcp__cbim__workflow_*']` | `[/Write/, /Edit/, /Bash/]` | 知识 CRUD + workflow 全 CRUD + 只读源码参考 |
| **programmer** | `['Read', 'Write', 'Edit', 'Bash', 'Glob', 'Grep', 'mcp__cbim__module_get', 'mcp__cbim__workflow_list', 'mcp__cbim__workflow_get', 'mcp__cbim__source_*', 'mcp__cbim__run_*', 'mcp__cbim__git_*']` | — | 只读知识 + workflow list/get + 源码全操作 + 构建测试 |
| **auditor** | `['Read', 'Glob', 'Grep', 'mcp__cbim__module_get', 'mcp__cbim__workflow_list', 'mcp__cbim__workflow_get']` | `[/Write/, /Edit/, /Bash/]` | 只读知识 + 只读 workflow + 只读源码 |
| **hr** | `['mcp__cbim__agent_*', 'mcp__cbim__skill_*', 'mcp__cbim__memory_query']` | — | agent/skill 全 CRUD + 记忆查询 |

### 8.3 为什么 SDK 配置优于路径守卫
...
6. **Workflow tool 是 module tool 的延伸**：workflow 跟着模块走，所以 workflow 操作都带 `modulePath` 参数
7. **Skill tool 与 workflow tool 对称**：这是 capability-business symmetry 的体现——同样的动词（list/get/create/update/delete），对称的参数结构
...
const cbimServer = createSdkMcpServer({
  ...
  tools: [
    ...
    // ... ~38 个 cbim_* tools（含 cbim_skill_* 5 个 + cbim_workflow_* 5 个）
  ],
})
