...
|-------|-------|-----------|
| architect | Read, Write, Edit, Glob, Grep, Bash | 限 `.aiworkspace/` |
...
项目信息存储在 `config/projects.json`。`/new-project <name> <path>` 命令自动完成注册并触发架构师初始化根模块。
...
**铁律**：能力进 `.claude/agents/`，内容进 `.aiworkspace/`，混入对方的内容一律不得升格。
