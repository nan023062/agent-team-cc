...
| 任务 | 用途 | 建议频率 |
|------|------|---------|
| 日常信号采集 | 扫描短期记忆，提取 agent / 模块升格信号 | 每日 |
| 周度升格 | 将信号落地为 skill / soul / `.aimodule/` 更新 | 每周 |
...
│       │   └── SKILL.md              ← 人力治理接口
│       └── memory/                   ← 记忆 skill
│           ├── SKILL.md              ← 记忆操作接口（含三层架构）
│           ├── daily-signal.md       ← 短期→中期提炼：信号采集
│           └── weekly-assessment.md  ← 短期→中期提炼：升格执行
│
