...
**明文文件是唯一数据源**，ChromaDB 仅作向量索引。

| 内容 | 存储 | git |
|------|------|-----|
| entry 原文 | `memory/entries/YYYY-MM-DD-<agent>-<slug>.md` | ✅ 提交 |
...
| 时机 | Hook | 行为 |
|------|------|------|
| 每次 session 启动 | `SessionStart` → `load-memory.py` | 查近期 entry，注入为主 agent 上下文 |
| 每次 session 结束 | `Stop` → `write-memory.py` | 解析 transcript，自动写入 entry |

Entry 记录本次调度了哪些 subagent、各自的任务、结果、遇到的问题。

**主 agent 是唯一的记忆持有者。** Subagent 专注执行，记忆由主 agent 统一管理。

向量查询（session 中途按需）：主 agent 读取 `.claude/skills/memory/SKILL.md` 执行，查询接口封装在 skill 内部。

`memory_index.py` 仅用于首次安装或索引损坏时手动重建。

| 场景 | 配置 |
|------|------|
| 本地开发 | 不设环境变量，索引存 `memory/chroma_db/` |
| 团队服务器 | 设置 `CHROMA_HOST` 和 `CHROMA_PORT` |

索引可随时从 `memory/entries/` 完整重建，丢失无损失。
