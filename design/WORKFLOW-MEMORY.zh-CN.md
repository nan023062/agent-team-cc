# CBIM 主 agent 的记忆能力（CRUD + 治理 双子循环流程图）

> 关联文档：[`LOOPS-OVERVIEW.zh-CN.md`](./LOOPS-OVERVIEW.zh-CN.md)（位置图）、[`WORKFLOW-EXECUTION.zh-CN.md`](./WORKFLOW-EXECUTION.zh-CN.md)（执行根，挂 CRUD 子循环流程图）、[`WORKFLOW-DREAM.zh-CN.md`](./WORKFLOW-DREAM.zh-CN.md)（治理根，挂治理子循环流程图）。

---

## 一句话定位

**主 agent 持有记忆 CRUD 子循环流程图和记忆治理子循环流程图；记忆服务本身没有流程图。**

- CRUD 子循环流程图：挂在执行根（用户驱动）下，负责业务路径上的读写。
- 治理子循环流程图：挂在治理根（scheduler 驱动）下，负责定期把存储层的维护动作叫起跑一次。
- 两条子循环流程图的主体都是**主 agent**——是 actor，是循环的持有者。

---

## 记忆服务边界

记忆服务是**项目本地的存储层**，只有内部状态、没有流程图、不持有定时器、不主动通知任何方；谁调用就执行一次。循环的主体永远是调用它的主 agent。

---

## 主 agent 记忆 CRUD 子循环流程图
...
| 节点 | 职责 |
|------|------|
| 治理根第一步触发 | 由治理根作为首个治理步骤拉起本子循环流程图 |
...
| 触发场景 | 走哪个子循环流程图 |
|---------|----------------|
| 一次用户对话结束（Hook 落条目） | CRUD 子循环流程图 |
| 新开会话拉历史上下文（Hook 召回） | CRUD 子循环流程图 |
| 用户显式说"记下这条" | CRUD 子循环流程图 |
| 用户显式说"上次我们决定的 X" | CRUD 子循环流程图 |
| 执行根批写出口冲刷 | CRUD 子循环流程图 |
| 执行根任一节点在业务路径上需要历史 | CRUD 子循环流程图 |
| 治理根定期拉起第一步治理 | 治理子循环流程图 |
| 人工运行维护命令 | 治理子循环流程图（共用同组维护接口） |

---

## 两条子循环流程图的关键差异

| 维度 | CRUD 子循环流程图 | 治理子循环流程图 |
|------|----------------|---------------|
| 挂在哪个根 | 执行根 | 治理根 |
| 节奏 | 业务路径上随调随跑 | 定期跑一次 |
...
两条子循环流程图**完全解耦**：CRUD 写入产生的候选不会回调治理子循环流程图，治理子循环流程图跑完也不会通知 CRUD 子循环流程图。这正是记忆服务"被动存储层"定位的体现。
